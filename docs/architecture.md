...
{
  "schemaVersion": 1,
  "lazyNvim": { "inputName": "lazy-nvim", "synthetic": true },  // always present. If the spec
                                          // lists lazy.nvim itself (#49), this carries the same
                                          // ref fields ("source"/"branch"/"tag"/"commit"/"version"/
                                          // "pin"/"resolvedRef") an ordinary plugin entry does, and
                                          // "synthetic" is false -- see below
  "plugins": {
  ...
  "localPlugins": { "myplugin": { } },  // dev/dir plugins: not locked. Only the name is recorded (#56); make-env reads the *names* and routes them to devPath. A plugins.json committed before #56 may still carry a "dir" here -- make-env has never read it, so that changes nothing
  "warnings": [ "..." ]
}
```

`localPlugins` here means any plugin lazy treats as local, e.g. a spec's own `dev = true` plugins
or a plugin whose spec sets `dir` without `dev` (#47): extract.lua tests the "is this dir under
lazy's own root" predicate lazy itself uses for `_.is_local` -- the same expression, though against
extraction's own sandbox root rather than the user's, so it is close to lazy's verdict and not
identical to it (see extract.lua's comment on `local_dir`) -- not whether the spec wrote `dev`, so
these shapes land in the same place and neither ever reaches `plugins` or a flake input.
...
- If the spec lists lazy.nvim itself (any plugin whose name -- the one lazy derives, or an explicit `name` -- is `lazy.nvim`; a fork's URL included, not just `folke/lazy.nvim`, #49), that entry decides *this* input rather than adding a second one -- `plugins` never gets a `"lazy.nvim"` key, because `nix/lib/make-env.nix`'s farm can only ever host one `lazy.nvim` entry (a second would silently replace nvimx's own foundation, `linkFarm` being last-wins). `build` and `dependencies` are dropped rather than recorded: nothing downstream reads either for lazy.nvim, and a spec `build` is warned about (`` `build` is ignored ``) rather than run. A plugin whose *name* merely normalizes to the reserved input name `lazy-nvim` (e.g. `{ "someone/lazy-nvim" }`) is a different case entirely and fails the lock, naming the plugin -- see "Plugin name collision" below
...
| explicit git URL | `git+https://` / `git+http://` / `git+ssh://` / `git+git://` / `git+file://...?ref=...&rev=...` -- only an exact `https://github.com/<owner>/<repo>` (or that same path with one trailing `/`) lands in the github type; every other host, and every non-https scheme even on github.com itself, stays git type | follows the ref unless a rev is pinned |

The URL shapes in this table, and the precedence between them -- one ladder on the github side, two independent slots on the git side -- are pinned executable-fashion by `tests/fixtures/spec-matrix/golden/matrix.flake.nix` and `.../priority.flake.nix` (`checks.resolve-golden` and `checks.genflake-golden`, #29). `pin = true` is deliberately in neither: freezing a rev needs a prior lock, which neither golden has. `lazyNvim` follows this same table when the spec lists lazy.nvim itself (#49, `checks.resolve-lazy-self`); unspecified is what a non-spec (synthetic) `lazyNvim` always resolves to.

**Source URL validation** (#28): `lua/nvimx/source.lua` classifies and validates `p.url` at lock time, before `plugins.json` is ever written. Accepted: `https://github.com/<owner>/<repo>` (github type, unchanged from before this validation existed -- https-only, host case-sensitive, and an existing trailing `/` on `repo` is kept rather than stripped); any other `https://`, `http://`, `ssh://` or `git://` URL with a host and a path -- the host's authority must not name a non-numeric port (`ssh://git@h:o/r.git`, the classic mistake of mechanically prefixing an scp form with `ssh://`, is rejected rather than degrading to a `path:` input later) and must not carry a character nix's flake-ref grammar itself rejects (verified against `builtins.parseFlakeRef`) -- or a `file://` URL with an empty authority and an absolute path (`file:///...`) (git type, stored byte-for-byte as written); and the scp-style `user@host:path` shorthand, `user@` mandatory (git type, normalized to `ssh://user@host/path` -- the one shape this file rewrites, because `git@host:path` is not a valid nix flake ref and would otherwise degrade to a `path:` input that only fails later, at build time; a colon-bearing string with no `user@`, e.g. `host:8080`, is not this shorthand and is rejected outright rather than silently rewritten). Everything else is fatal: a lock-time error naming the plugin and the offending URL, and no `plugins.json` is written. `?`, `#` and `${` are always rejected in the source URL itself -- `genflake.lua` owns the query string, building `?ref=` / `?rev=` from `branch` / `tag` / `commit` itself, so a source URL that already carries one would double up into an unparseable flake ref, and it writes the URL into `flake.nix` with Lua's `%q`, which does not escape `${`, so one would otherwise become a live Nix string interpolation instead of literal text. A URL naming just a host and no repository path is also rejected, whether or not it has a trailing `/` (`https://git.example.com` and `https://git.example.com/` both fail); a `https://github.com/<owner>` with no second path segment does not match the github type at all, and instead falls through to this same generic-git handling unchanged, which does accept it (this file classifies, it does not decide whether a plugin can actually be cloned).

Two migration notes, both from this validation replacing a previous silent pass-through:

- A plugin on the scp shorthand (`git@host:owner/repo.git`) gets its `resolvedRef` discarded exactly once, the first time it locks under this validation, because `source.url` changing to the `ssh://` form flips the plugin's spec identity. A `resolvedRef` derived from `git ls-remote` (semver / a moved branch or tag) is usually recomputed to the same value on the next lock; one seeded by `--import-lazy-lock` as a bare 40-hex commit is not re-derived and is simply gone. `git@host:path`'s `path` is also home-relative for the ssh user, while `ssh://host/path` is absolute -- irrelevant for GitHub/GitLab/Gitea/Forgejo/sourcehut, which treat both the same, but a self-hosted server that serves repositories out of the ssh user's home directory (`git@myhost:repos/x.git` = `~/repos/x.git`) now points somewhere else; write `url = "ssh://git@myhost/~/repos/x.git"` explicitly to keep the old target.
- A git-type source URL that already carries `?ref=` / `?rev=` (and does not also set `branch` / `tag` / `commit` -- that combination was never valid, since `genflake.lua` would double the query) now fails the lock outright, since a source URL is no longer allowed to carry a query string at all. Move the query into the spec fields instead: `?ref=<x>` becomes `branch = "<x>"` or `tag = "<x>"`, `?rev=<sha>` becomes `commit = "<sha>"`.

github.com over ssh or the scp shorthand (`git@github.com:owner/repo.git`) is deliberately *not* folded into the github type: doing so would mean every fetch of that plugin needs an ssh key (the tarball API the github type otherwise uses does not accept one), breaking anyone pinning a private repo that way. It stays git type, with the tradeoff that a *public* repo written this way loses the github type's lighter tarball fetch and needs a working ssh transport instead of nothing at all.
...
4. **dev.path is a function**: `make-env.nix` bakes a name → directory table into `bootstrap.lua` (`devDirs`). Every name it holds — from `devPlugins`, or a key of the lock's `localPlugins` (e.g. a spec's own `dev = true` plugins, or one whose spec sets `dir` without `dev`, #47) — maps to `<devPath>/<name>`; anything else falls back to `<farm>/<name>`. `devPath` is the single knob for the entries this table decides, and it replaces lazy's own `dev.path`, which the forced opts override anyway. `localPlugins[*].dir` is **not** read, because reading it could not change any resolved directory: a `dir` the user wrote in the spec short-circuits lazy before `dev.path` is consulted at all (`meta.lua:214-217`), so the only entries the table can decide are the bare `dev = true` ones. The lock itself records only the name (#56): there is no recorded path left to have an effect on another machine. A `plugins.json` committed before #56 may still carry a `dir` here, and it is ignored the same way — this code has never read it. The function must return the **full** plugin directory: lazy appends `/<name>` only to the string form (`lua/lazy/core/meta.lua:229-231`). `fallback = false` is kept, so a working tree that does not exist is *not* silently replaced by the store copy. A name in neither the lock nor `localPlugins` is reported as `unknownDevPluginNames` and warned about at activation time, like `unknownPluginNames`
...
lua/nvimx/
  ...
  source.lua                 # pure: plugin source URL classification, normalization, validation (#28)
...
tests/fixtures/              # basic-config / build-plugins / build-steps-config / registry-plugins / cargo-git-lock / treesitter-config / unbuildable-config / local-plugin / empty-config / merge / merge-config / defaults-version-config / defaults-version-false-config / local-dir-config / semver / update / import-lazy-lock / dev-plugins / source-urls / spec-matrix / lazy-self / golden/
```
...
| Non-GitHub / explicit git URL | validated at lock time (#28, `lua/nvimx/source.lua`): `https://` / `http://` / `ssh://` / `git://` with a host and path, or `file://` with an empty authority and an absolute path (`file:///...`), and the scp shorthand `user@host:path` (normalized to `ssh://user@host/path`), all lock to a `git+...` input; anything else -- a local path, an unknown scheme, a query string / fragment / `${`, a non-numeric port or grammar-invalid character in the authority, a `https://github.com/<owner>/<repo>/...` path with more than the two segments the github type can express -- fails the lock naming the plugin and the URL, before `plugins.json` is written. A github.com path with *fewer* than two segments (`https://github.com/<owner>`, no repo at all) does not match the github type in the first place and is instead accepted as a generic git URL, unchanged from before this validation existed |
| Plugin name collision | surfaces during lazy's Spec normalization (same behavior as lazy). Two plugins whose derived flake input names collide with each other fail the lock at lock time, naming both plugins (`checks.resolve-lazy-self`, #49); `lazy-nvim` is additionally a reserved input name (lazy.nvim itself always claims it), so any other plugin whose name normalizes to it also fails the lock the same way, naming that plugin. This is unrelated to a spec listing lazy.nvim itself, which is not a collision at all -- see the plugins.json schema section above |
...
| Local plugin development | `devPlugins` names them, `devPath` says where they live, and the dev.path function routes them; a spec-level `dev = true` is picked up automatically from `localPlugins` and follows `devPath` too. A spec entry that writes `dir` itself (with or without `dev`, #47) is likewise picked up automatically from `localPlugins`, but lazy short-circuits on that `dir` before `devPath` is ever consulted, so `devPath` never applies to it. A plugin named in `devPlugins` stays in the lock and in the farm, so removing the name restores the pinned build with no re-lock; a spec-level `dev = true` or `dir` plugin was never locked in the first place. A missing working tree is not fallen back on |
