...
`localPlugins` here means both a spec's own `dev = true` plugins and a plugin whose spec sets `dir`
without `dev` (#47): extract.lua tests the "is this dir under lazy's own root" predicate lazy itself
uses for `_.is_local` -- the same expression, though against extraction's own sandbox root rather
than the user's, so it is close to lazy's verdict and not identical to it (see extract.lua's comment
on `local_dir`) -- not whether the spec wrote `dev`, so the two shapes land in the same place and
neither ever reaches `plugins` or a flake input.
